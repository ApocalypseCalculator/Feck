...
module.exports.execute = function (req, res) {
    ...
    if (req.headers["tus-resumable"] && req.headers["tus-resumable"] == "1.0.0") {
        if (!req.headers["upload-defer-length"]) {
            ...
            else if (!req.headers["base64-meta"]) {
                return res.status(400).json({ status: 400, error: 'Missing Base64-Meta header' });
            }
            else {
                try {
                    req.body = JSON.parse(Buffer.from(req.headers["base64-meta"], 'base64').toString());
                }
                catch {
